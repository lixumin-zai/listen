...
def make_generator_model():
    ...
    model.add(layers.Dense(1*25*128, use_bias=False, input_shape=(100,)))


    model.add(layers.Reshape((1, 25, 128)))
    assert model.output_shape == (None, 1, 25, 128)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())

    model.add(layers.Conv2DTranspose(64, (1, 5), strides=(1,2), padding='same', use_bias=False))
    assert model.output_shape == (None, 1, 50, 64)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    #
    model.add(layers.Conv2DTranspose(32, (1, 5), strides=(1,2), padding='same', use_bias=False))
    assert model.output_shape == (None, 1, 100, 32)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    # #
    model.add(layers.Conv2DTranspose(16, (2, 1), strides=(2, 1), padding='same', use_bias=False))
    assert model.output_shape == (None, 2, 100, 16)
    model.add(layers.BatchNormalization())
    model.add(layers.LeakyReLU())
    #
    model.add(layers.Conv2DTranspose(1, (2, 1), strides=(2, 1), padding='same', use_bias=False, activation='tanh'))
...
def make_discriminator_model():
    ...
    model.add(layers.Conv2D(128, (1,5), strides=(1,2), padding='valid'))
    model.add(layers.LeakyReLU())
    model.add(layers.Dropout(0.1))
...
checkpoint_dir = './training_checkpoints_main2.1'
